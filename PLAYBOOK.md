...
> 이 저장소(구리 출장마사지·홈타이 / 간다GO) 구조를 그대로 복제해
> 다른 지역(예: 남양주, 하남, 의정부)에도 동일한 사이트를 만들 수 있습니다.
> 핵심 설계: **행정구가 없는 시 → 대표 행정동 → 지하철역**, 페이지 수를 과하게 늘리지 않는 린(lean) 구조.
...
## 2. 페이지 구성 (이 템플릿 = 15페이지)

```
메인 1                      /
대표 행정동 5               /guri/{dong}-chuljangmassage/      (예: galmae-dong, gyomun-dong)
지하철역 4                  /guri/{station}-chuljangmassage/   (예: guri-station, donggureung-station)
예약 안내                   /reservation/
이용 전 확인사항            /guide/
홈타이 이용 가이드          /hometai/
...
상단 메뉴(NAV)는 짧게: 하위 메뉴엔 키워드 반복 없이 **동·역 이름만**.
행정동·역 그룹의 부모는 페이지가 아니라 메인 앵커(`/#areas`, `/#stations`)로 연결.
